...
The dataset consists of retail data (~313 MB) and is excluded from this repo due to file size. It is available for download via Google Drive:

📂 **[Download dataset (Google Drive)](https://drive.google.com/drive/folders/1ObeASgGRMPCVU4u4JxX7PXSLmQTOenNr?usp=sharing)**

Place all files in the `data/` folder before running the notebook. The dataset includes:
...
```
promotion-response-prediction/
├── data/                        # Raw data files (excluded from repo — see Google Drive link above)
...
├── notebook/
│   └── promotion_response_model.ipynb   # Full pipeline with visualisations
├── report/
...
1. Download the dataset from the [Google Drive link](https://drive.google.com/drive/folders/1ObeASgGRMPCVU4u4JxX7PXSLmQTOenNr?usp=sharing) above
2. Upload `promotion_response_model.ipynb` to Google Colab
3. Upload the data files to the Colab session storage under `data/`
4. Run all cells — total runtime is under 10 minutes
5. Charts are saved as PNG files and downloaded automatically via the final cell
